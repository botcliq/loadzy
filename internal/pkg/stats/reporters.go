...
func NewTextReporter(m *Metrics) Reporter {
	...
	return func(w io.Writer) (err error) {
		...
		fmt.Fprintln(tw, "Slowest responses:\t")
		for i := len(m.Slowest) - 1; i >= 0; i-- {
			if m.Slowest[i].Latency > 0*time.Second {
				fmt.Fprintf(tw, "Req# %7d |TimeStamp %14s | %4v | latency %v | %s\n", m.Slowest[i].Seq, m.Slowest[i].Timestamp.String(), m.Slowest[i].Code,
					m.Slowest[i].Latency, m.Slowest[i].Body)
			}
		}
		mutex.Unlock()
		return tw.Flush()
